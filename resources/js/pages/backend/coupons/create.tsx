...
type CouponForm = {
    title: string; subtitle: string; image: File | null; type: string; code: string;
    value: number | string; min_night: number | string; starts_at: string; ends_at: string; is_active: boolean;
};

export default function Create() {
    const { data, setData, post, processing, errors } = useForm<CouponForm>({
        title: "", subtitle: "", image: null, type: "percentage", code: "", value: 0,
        min_night: 0, starts_at: "", ends_at: "", is_active: true,
    });
    const submit = (event: React.FormEvent) => { event.preventDefault(); post("/admin/coupons", { forceFormData: true }); };
    const fieldError = (field: keyof CouponForm) => errors[field] && <div className="text-danger mt-1">{errors[field]}</div>;

    return <><Head title="Create Coupon" /><div className="card bg-white border border-white rounded-10 p-20"><h3 className="mb-20">Create Coupon</h3><form onSubmit={submit}>
        ...
        </div><div className="col-lg-4"><ImageUpload label="Coupon image" file={data.image} onChange={file => setData("image", file)} />{fieldError("image")}</div></div>
        <button type="submit" className="btn btn-primary text-white" disabled={processing}>Create coupon</button>
    </form></div></>;
}
