...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm<AmenityForm>({
        name: "",
        icon: null,
        is_active: true,
    });
    ...
    const submit = (event: React.FormEvent) => {
        ...
        post("/admin/amenities", { forceFormData: true });
    };
    ...
    return (
        <>
            ...
            <div className="card bg-white border border-white rounded-10 p-20">
                <h3 className="mb-20">Add amenity</h3>
                <form onSubmit={submit}>
                    <div className="row">
                        ...
                        <div className="col-lg-5">
                            <ImageUpload label="Amenity icon" file={data.icon} onChange={file => setData("icon", file)} />
                            {errors.icon && <div className="text-danger mt-1">{errors.icon}</div>}
                        </div>
                    </div>
                    <button type="submit" className="btn btn-primary text-white" disabled={processing}>Add amenity</button>
                </form>
            </div>
        </>
    );
